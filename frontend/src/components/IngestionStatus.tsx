...
export default function IngestionStatus() {
  const { ingestionJob, setIngestionJob, selectedRepo, setSelectedRepo,
          setScopeRepos, setRepos } = useGraphStore();
      ...
      >
        <div className="flex items-start gap-3">
          <div className="mt-0.5">
            {isRunning && <Loader2 className="w-4 h-4 text-[#9b7a31] animate-spin" />}
            {isCompleted && <CheckCircle2 className="w-4 h-4 text-[#315b47]" />}
            {isFailed && <AlertCircle className="w-4 h-4 text-[#a45138]" />}
          </div>
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-[#8b929e] mt-1 truncate">
              {ingestionJob.message}{dots}
            </p>

            {isFailed && ingestionJob.error && (
              <p className="text-xs text-red-600 mt-1">{ingestionJob.error}</p>
            )}

            <div className="mt-2">
              <div className="w-full h-1.5 rounded-full overflow-hidden" style={{ background: "#dfe2e8" }}>
                <div
                  className="h-full rounded-full transition-all duration-500"
                  style={{
                    width: `${ingestionJob.progress}%`,
                    background: isCompleted ? "#315b47" : isFailed ? "#a45138" : "#9b7a31",
                  }}
                />
              </div>
              <p className="text-xs text-[#8b929e] mt-1">{ingestionJob.progress}%</p>
            </div>
          </div>
        </div>
      </div>
