        ...
        <div className="space-y-4 bg-black/20 p-4 rounded-xl border border-white/5">
          <div>
            ...
            <input
              type="text"
              list="trace-services"
              placeholder={services[0] ? `e.g. ${services[0]}` : "e.g. api-gateway"}
              value={fromSvc} onChange={e => setFromSvc(e.target.value)}
              className="w-full bg-[#2b313a] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-violet-500"
            />
          </div>
          <div>
            ...
            <input
              type="text"
              list="trace-services"
              placeholder={services[1] ? `e.g. ${services[1]}` : "e.g. users-db"}
              value={toSvc} onChange={e => setToSvc(e.target.value)}
              className="w-full bg-[#2b313a] border border-white/10 rounded-lg px-3 py-2 text-sm text-white outline-none focus:border-violet-500"
            />
            <datalist id="trace-services">
              {services.map((n) => <option key={n} value={n} />)}
            </datalist>
          </div>
