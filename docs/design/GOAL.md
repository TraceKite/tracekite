...
gap. So Adduce emits **claims** from each artifact independently, then joins
...
At the end Adduce is **two surfaces over one engine**:

- `docker compose up` — the product, exactly as today
- `pip install adduce-core` — the same engine as a library: no server, no
...
3. **Model** (10) — UI_CALLS, vendor catalog, SCIP import for symbol calls,
   `adduce explain`. *Exit: every edge explains its own derivation.*
