# Pre-redesign TraceKite UI browser review
...
- Repository scope with `TraceKite/tracekite` and `sfbayman/lanovyx` selected.
...
- Distributed Trace from `tracekite/frontend-external` to `tracekite/backend`, which
