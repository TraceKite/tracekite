...
bug in Adduce, not a break. So each loss carries which side's repositories
...
from adduce.services.linker.history import changed_between, edge_key
        ...
        if attributing and not provider_moved and not consumer_moved:
            result.unexplained.append({
                ...
                "why": "neither side's repository changed in this branch, so "
                       "this edge disappeared for a reason inside Adduce",
            })
...
def as_comment(result: Impact, *, limit: int = 20) -> str:
    ...
    if not blocking and not result.unexplained:
        return ("**Adduce**: no consumer loses a connection in this change "
    ...
    risk = result.risk()
    lines = [f"**Adduce**: {len(groups)} connection(s) lost across "
    ...
    if result.unexplained:
        lines.append(
            ...
            "side changing — that is a regression in Adduce, not in this "
            "branch._")
    return "\n".join(lines)
