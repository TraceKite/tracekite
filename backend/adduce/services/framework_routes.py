...
def computed_go_paths(content: str) -> int:
    ...
    """
    from adduce.services.go_route_extractor import _scan, _imports
...
def framework_routes(file_info, content: str) -> tuple[list, int]:
    ...
    if language in ("javascript", "typescript") or path.endswith(
            (".ts", ".tsx", ".js", ".jsx", ".mjs")):
        from adduce.services.js_route_extractor import extract_js_routes
    ...
    if path.endswith(".pb.gw.go"):
        ...
        from adduce.services.grpc_gateway_routes import extract_gateway_routes
        return extract_gateway_routes(path, content), 0
    if language == "go" or path.endswith(".go"):
        from adduce.services.go_route_extractor import extract_go_routes
