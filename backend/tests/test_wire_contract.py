...
`schemas/adduce-wire-<version>.json` is what a non-Python host validates
...
regenerate: `python -m adduce.cli schema > schemas/adduce-wire-<version>.json`.
"""
...
from adduce.wire import PUBLISHED, WIRE_VERSION, json_schemas

SCHEMA_DIR = os.path.join(
    os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))),
    "schemas")
PUBLISHED_PATH = os.path.join(SCHEMA_DIR, f"adduce-wire-{WIRE_VERSION}.json")
...
class TestPublished:
    def test_schema_file_exists_for_this_version(self):
        assert os.path.exists(PUBLISHED_PATH), (
            ...
            f"  python -m adduce.cli schema > {PUBLISHED_PATH}")
    ...
    def test_filename_carries_the_version(self):
        ...
        match = re.fullmatch(r"adduce-wire-(\d+\.\d+\.\d+)\.json", name)
