...
class TestAuth:
    ...
    def test_health_is_public(self):
        with patch("adduce.routes.health.check_neo4j_health", return_value=True):
    ...
    def test_valid_token_passes(self, auth_headers):
        with patch("adduce.routes.repos.list_repos", return_value=[]):
            response = client.get("/api/repos", headers=auth_headers)
        assert response.status_code == 200
    ...
    def test_anonymous_read_allowed_when_enabled(self):
        with patch.object(settings, "allow_anonymous_reads", True), \
             patch("adduce.routes.repos.list_repos", return_value=[]):
            response = client.get("/api/repos")
        assert response.status_code == 200
    ...
    def test_health_reports_anonymous_read_mode(self):
        with patch.object(settings, "allow_anonymous_reads", True), \
             patch("adduce.routes.health.check_neo4j_health", return_value=True):
...
class TestHealthRoute:
    ...
    def test_health_degraded(self):
        with patch("adduce.routes.health.check_neo4j_health", return_value=False):
            body = client.get("/health").json()
...
class TestReposRoutes:
    def test_ingest_queues_job(self, auth_headers):
        with patch("adduce.routes.repos.job_queue") as queue:
    ...
    def test_list_repositories(self, auth_headers):
        with patch("adduce.routes.repos.list_repos",
    ...
    def test_get_repository_found(self, auth_headers):
        with patch("adduce.routes.repos.get_repo", return_value=_summary()):
...
class TestJobsRoute:
    def test_job_status_found(self, auth_headers):
        ...
        with patch("adduce.routes.jobs.get_session", return_value=ctx):
...
class TestGraphRoutes:
    def test_get_graph(self, auth_headers):
        ...
        with patch("adduce.routes.graph.get_graph", return_value=fake):
    ...
    def test_search(self, auth_headers):
        with patch("adduce.routes.graph.search_nodes", return_value=[]):
            response = client.get("/api/repos/r1/search?q=foo",
                                  headers=auth_headers)
        assert response.status_code == 200

    def test_node_detail_missing(self, auth_headers):
        with patch("adduce.routes.graph.get_node_details", return_value=None):
    ...
    def test_node_detail_found(self, auth_headers):
        ...
        with patch("adduce.routes.graph.get_node_details", return_value=detail):
