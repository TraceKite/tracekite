...
class TestLinksRebuild:
    def test_rebuild_queues_link_full(self, auth_headers):
        with patch("tracekite.routes.links.job_queue") as queue:
...
class TestLinksStatus:
    def test_status_with_stale_repos(self, auth_headers):
        ...
        with patch("tracekite.routes.links.list_link_runs", return_value=runs), \
             patch("tracekite.routes.links.get_session",
    ...
    def test_status_without_stale_repos(self, auth_headers):
        ...
        with patch("tracekite.routes.links.list_link_runs", return_value=[]), \
             patch("tracekite.routes.links.get_session",
                   return_value=_session_ctx(session)):
            body = client.get("/api/v2/links/status", headers=auth_headers).json()
        assert body["runs"] == []
...
class TestTrace:
    def test_resolve_unknown_returns_404(self, auth_headers):
        ...
        with patch("tracekite.routes.trace.get_session",
                   return_value=_session_ctx(session)):
            response = client.get(
...
class TestAuthMiddleware:
    def test_rate_limiter_window_eviction(self):
        from tracekite.middleware.auth import RateLimiter
        clock = {"t": 100.0}
        with patch("tracekite.middleware.auth.time.monotonic",
    ...
    def test_audit_swallows_oserror(self):
        from tracekite.middleware.auth import audit
        with patch("tracekite.middleware.auth.open",
                   side_effect=OSError("disk full")), \
             patch("tracekite.middleware.auth.os.makedirs"), \
             patch("tracekite.middleware.auth.logger") as log:
    ...
    def test_trace_returns_429_when_rate_limited(self, auth_headers):
        ...
        with patch("tracekite.middleware.auth._limiter.allow", return_value=False), \
             patch("tracekite.routes.trace.get_session",
                   return_value=_session_ctx(session)):
            response = client.get(
...
class TestRateLimitBuckets:
    ...
    def test_rebuild_has_its_own_tighter_bucket(self):
        from tracekite.middleware.auth import _rate_bucket
