...
JSON on stdout and nothing else, so `tracekite link ... | jq` works.
"""
...
def run(*args):
    proc = subprocess.run(
        [sys.executable, "-m", "tracekite.cli", *args],
...
class TestFullDerivation:
    ...
    def _edges(self):
        from tracekite.cli_explain import _derivation
...
class TestMeasuredInExplain:
    ...
    def test_a_priced_edge_carries_its_interval(self):
        from tracekite.cli_explain import _measured_row
