...
from evigraph.agent_setup import (
    SKILL_CONTENT,
    SkillConflictError,
    install_skills,
)
...
def test_codex_and_kimi_use_their_published_skill_locations(tmp_path):
    ...
    assert installed == [
        (tmp_path / ".agents/skills/evigraph/SKILL.md", "Codex"),
        (tmp_path / ".kimi/skills/evigraph/SKILL.md", "Kimi"),
    ]
...
def test_existing_matching_skill_is_verified_without_rewrite(tmp_path):
    path = tmp_path / ".claude/skills/evigraph/SKILL.md"
...
def test_conflict_preflight_prevents_partial_install(tmp_path):
    conflict = tmp_path / ".claude/skills/evigraph/SKILL.md"
    ...
    assert not (tmp_path / ".agents/skills/evigraph/SKILL.md").exists()
...
def test_only_published_client_locations_are_selected(tmp_path):
    ...
    assert paths == {
        Path(".claude/skills/evigraph/SKILL.md"),
        Path(".agents/skills/evigraph/SKILL.md"),
        Path(".kimi/skills/evigraph/SKILL.md"),
        Path(".gemini/config/skills/evigraph/SKILL.md"),
    }
...
def test_cross_client_plugin_contains_the_published_skill_and_mcp_server():
    ...
    assert (PLUGIN / "skills/evigraph/SKILL.md").read_text(
        encoding="utf-8") == SKILL_CONTENT
    assert codex["mcpServers"]["evigraph"]["args"] == ["mcp"]
    assert claude["mcpServers"]["evigraph"]["args"] == ["mcp"]
    assert kimi["mcpServers"]["evigraph"]["args"] == ["mcp"]


def test_plugin_and_package_versions_are_released_together():
    application = tomllib.loads((REPO / "pyproject.toml").read_text())
    core = tomllib.loads(
        (REPO / "packaging/evigraph-core/pyproject.toml").read_text())
    versions = {
