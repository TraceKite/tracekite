...
class TestJobHandlers:
    ...
    def test_handle_link_full(self):
        ...
        with patch("tracekite.services.linker.LinkerService", fake_service), \
             patch("tracekite.services.job_handlers.create_or_update_job") as cj:
...
class TestJobQueue:
    def test_register_handler_rejects_unknown(self):
        from tracekite.services.job_queue import JobQueue
        q = JobQueue()
    ...
    def test_submit_and_dedupe(self):
        from tracekite.services.job_queue import JobQueue
        q = JobQueue()
        with patch("tracekite.services.job_queue.create_or_update_job") as cj:
    ...
    def test_worker_no_handler_marks_failed(self):
        from tracekite.services.job_queue import JobQueue, Job
        q = JobQueue()
    ...
    def test_reap_stale_jobs(self):
        ...
        with patch("tracekite.db.neo4j_client.get_session", return_value=_ctx(session)):
