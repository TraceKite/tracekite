...
from tracekite.db.memory_store import claim_record
...
def _mapped_fields() -> set[str]:
    ...
    source = (BACKEND / "tracekite/db/memory_store.py").read_text()
...
class TestNoReaderBuildsItsOwn:
    def test_readers_route_through_the_shared_mapping(self):
        offenders = []
        for path in sorted(BACKEND.glob("tracekite/**/*.py")):
