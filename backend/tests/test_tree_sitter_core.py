...
Modules covered:
- adduce.parsers.tree_sitter.core.base_parser
- adduce.parsers.tree_sitter.core.queries.query_loader
- adduce.parsers.tree_sitter.core.parser
...
from adduce.parsers.tree_sitter.core.base_parser import BaseExtractor, BaseParser
from adduce.parsers.tree_sitter.core.models import LanguageType, ParsingResult
from adduce.parsers.tree_sitter.core.parser import TreeSitterParser
from adduce.parsers.tree_sitter.core.queries.query_loader import QueryLoader
...
class TestQueryLoader:
    def test_default_queries_dir(self):
        loader = QueryLoader()
        expected = Path(__file__).parent.parent / "adduce" / "parsers" / "tree_sitter" / "core" / "queries" / "files"
...
with patch.dict(sys.modules, {"tree_sitter": None}):
    import adduce.parsers.tree_sitter.core.parser as fallback_parser
...
class TestParseContent:
    def test_parse_content_success(self, monkeypatch):
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.TREE_SITTER_AVAILABLE", True)
        parser = TreeSitterParser()
    ...
    def test_parse_content_tree_sitter_unavailable(self, monkeypatch):
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.TREE_SITTER_AVAILABLE", False)
        parser = TreeSitterParser()
...
class TestGetParser:
    ...
    def test_get_parser_create_new(self, monkeypatch):
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.TREE_SITTER_AVAILABLE", True)
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.Language", FakeLanguage)
        parser = TreeSitterParser()
        root = MockNode("module", 0, 1, (0, 0), (0, 1), [])
        monkeypatch.setattr(parser, "_get_language", lambda lang, fp: FakeLanguage(object()))
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.Parser", MockParserFactory(root))
    ...
    def test_get_parser_version_incompatibility(self, monkeypatch):
        ...
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.Parser", raise_incompatible)
        assert parser._get_parser(LanguageType.PYTHON, "test.py") is None
    ...
    def test_get_parser_other_value_error_is_caught_outer(self, monkeypatch):
        ...
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.Parser", raise_other)
        assert parser._get_parser(LanguageType.PYTHON, "test.py") is None
    ...
    def test_get_parser_create_exception(self, monkeypatch):
        ...
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.Parser", raise_runtime)
        assert parser._get_parser(LanguageType.PYTHON, "test.py") is None
    ...
    def test_get_parser_outer_exception(self, monkeypatch):
        ...
        monkeypatch.setattr(
            "adduce.parsers.tree_sitter.core.parser.get_tree_sitter_name", raise_outer
        )
        assert parser._get_parser(LanguageType.PYTHON, "test.py") is None
...
class TestGetLanguage:
    ...
    def test_get_language_version_fallback(self, monkeypatch):
        monkeypatch.setattr("adduce.parsers.tree_sitter.core.parser.TREE_SITTER_AVAILABLE", True)
        monkeypatch.setattr(
            "adduce.parsers.tree_sitter.core.parser.Language", FakeLanguageVersionFallback
        )
        parser = TreeSitterParser()
        fake_module = types.ModuleType("tree_sitter_java")
        fake_module.language = lambda: object()
