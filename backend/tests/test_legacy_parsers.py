...
from adduce.parsers.base import ParsedImport, ParseResult
...
from adduce.parsers.docker_parser import (
    DockerResource,
...
'''
    ...
    def test_parse_config_file_exception(self):
        with patch.dict("adduce.parsers.config_parser.CONFIG_PARSERS", {".yml": lambda fp, c: (_ for _ in ()).throw(IOError("fail"))}):
...
<project>
    ...
    def test_parse_pom_xml_exception(self):
        with patch("adduce.parsers.dependency_parser.re.finditer", side_effect=RuntimeError("boom")):
...
'''
    ...
    def test_parse_dependency_file_exception(self):
        with patch.dict("adduce.parsers.dependency_parser.DEPENDENCY_PARSERS", {"requirements.txt": lambda fp, c: (_ for _ in ()).throw(IOError("fail"))}):
