...
Runs the labeled-estate harness in adduce/services/calibration.py, prints the
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        help="Control-plane directory holding confidence.yml (default: the "
             "repo's config/, resolved via adduce.services.linker.base."
             "config_dir()).")
    parser.add_argument(
    ...
    import adduce.config  # noqa: F401

    from adduce.services.calibration import run_calibration
    from adduce.services.calibration_io import (
        DEFAULT_CORPUS, coverage_gaps, write_measured,
    )
    from adduce.services.linker.base import config_dir, load_confidence
    from adduce.services.linker.reliability import (
