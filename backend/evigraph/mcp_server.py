...
SDK: `pip install evigraph-core` promises a light dependency set, and three
...
from evigraph.utils.evidence import as_spans
...
def handle(message: object, tools: GraphTools | None) -> dict | None:
    ...
    if method == "initialize":
        return _response(request_id, {
            ...
            "capabilities": {"tools": {}},
            "serverInfo": {"name": "evigraph", "version": "1"}})
...
def _collect_claims(path: str, claims: list, commits: dict) -> None:
    ...
    if os.path.isfile(path) and path.endswith(".evigraph"):
        meta = read_meta(path)
    ...
    if os.path.exists(path):
        raise ValueError(f"unsupported MCP input (expected .evigraph): {path}")
