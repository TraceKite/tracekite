"""`evigraph` on the command line: scan a repo, link an estate, print JSON.
...
whatever the CLI can do, a host can do, and neither has its own copy.
    ...
    python -m evigraph.cli scan  path/to/repo --repo-id myrepo
    python -m evigraph.cli link  path/to/a path/to/b
    python -m evigraph.cli explain path/to/a path/to/b --edge <source> <target>
...
from evigraph.agent_setup import SUPPORTED_CLIENTS, cmd_install_skill
from evigraph.cli_emit import emit as _emit
from evigraph.cli_explain import cmd_explain
from evigraph.cli_inspect import (
...
def cmd_artifact(args) -> int:
    """Scan one repository into a content-addressed .evigraph file.
...
def cmd_mcp(args) -> int:
    ...
    from evigraph.mcp_server import serve
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--hmac-key", default=os.environ.get(
        "GRAPH_HMAC_KEY", "evigraph-cli-local"),
    ...
    # contract is that `evigraph link ... | jq` works, and a log line in the
    ...
    parser.add_argument("--log-format", default=None, choices=["text", "json"],
                        help="default: json when EVIGRAPH_LOG_FORMAT=json")
    ...
    art_p = sub.add_parser(
        "artifact", help="scan into a content-addressed .evigraph file")
    ...
    mcp_p.add_argument(
        ...
        help=".evigraph artifacts or source directories (default: current directory)")
    mcp_p.set_defaults(func=cmd_mcp)

    install_p = sub.add_parser(
        "install-skill", help="install the global Evigraph agent skill")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from evigraph.run_logging import configure_logging, run_context
