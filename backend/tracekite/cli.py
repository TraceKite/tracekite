"""`tracekite` on the command line: scan a repo, link an estate, print JSON.
...
whatever the CLI can do, a host can do, and neither has its own copy.
    ...
    python -m tracekite.cli scan  path/to/repo --repo-id myrepo
    python -m tracekite.cli link  path/to/a path/to/b
    python -m tracekite.cli explain path/to/a path/to/b --edge <source> <target>
...
from tracekite.agent_setup import SUPPORTED_CLIENTS, cmd_install_skill
from tracekite.cli_emit import emit as _emit
from tracekite.cli_explain import cmd_explain
from tracekite.cli_inspect import (
...
def cmd_scan(args) -> int:
    ...
    from tracekite.wire import ScanReport
...
def cmd_artifact(args) -> int:
    """Scan one repository into a content-addressed .tracekite file.
...
def cmd_link(args) -> int:
    ...
    from tracekite.wire import LinkReport
...
def cmd_mcp(args) -> int:
    ...
    from tracekite.mcp_server import serve
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--hmac-key", default=os.environ.get(
        "GRAPH_HMAC_KEY", "tracekite-cli-local"),
    ...
    # contract is that `tracekite link ... | jq` works, and a log line in the
    ...
    parser.add_argument("--log-format", default=None, choices=["text", "json"],
                        help="default: json when TRACEKITE_LOG_FORMAT=json")
    ...
    art_p = sub.add_parser(
        "artifact", help="scan into a content-addressed .tracekite file")
...
def main(argv: list[str] | None = None) -> int:
    ...
    from tracekite.run_logging import configure_logging, run_context
