...
from tracekite.parsers.graphql_parser import DEFAULT_ROOTS, extract_gql_tags

logger = logging.getLogger(__name__)
from tracekite.services.grpc_extractor import extract_grpc_sites
from tracekite.services.file_classifier import classify
...
from tracekite.services.http_call_extractor import (
...
from tracekite.services.claim_sink import add_claim  # noqa: F401 (re-export)
from tracekite.services.ingest_config_defs import (
...
def emit_source_claims(repo_id: str, file_info, content: str,
    ...
    from tracekite.services.flag_guards import annotate_flag_guards
    ...
    from tracekite.parsers.iac_units import extract_iac_services
    from tracekite.services.ingest_artifacts import process_iac_units
    ...
    for line, url, deliverer in extract_webhook_registrations(content):
        from tracekite.services.http_call_url import classify_url
...
def emit_config_claims(repo_id: str, file_info, config_result, file_node_id: str,
    ...
    from tracekite.services.messaging_extractor import extract_stream_bindings
...
def emit_messaging_claims(repo_id: str, file_info, content: str,
    ...
    from tracekite.services.messaging_extractor import extract_messaging_sites
...
def emit_data_site_claims(repo_id: str, file_info, content: str,
    ...
    from tracekite.services.data_extractor import extract_data_sites
...
def emit_migration_claims(repo_id: str, file_info, content: str,
    ...
    from tracekite.parsers.migration_parser import is_migration_file, parse_migration
