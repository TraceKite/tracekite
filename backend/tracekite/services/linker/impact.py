...
bug in TraceKite, not a break. So each loss carries which side's repositories
        ...
        if attributing and not provider_moved and not consumer_moved:
            result.unexplained.append({
                ...
                "why": "neither side's repository changed in this branch, so "
                       "this edge disappeared for a reason inside TraceKite",
            })
...
def as_comment(result: Impact, *, limit: int = 20) -> str:
    ...
    if not blocking and not result.unexplained:
        return ("**TraceKite**: no consumer loses a connection in this change "
    ...
    risk = result.risk()
    lines = [f"**TraceKite**: {len(groups)} connection(s) lost across "
    ...
    if result.unexplained:
        lines.append(
            ...
            "side changing — that is a regression in TraceKite, not in this "
            "branch._")
    return "\n".join(lines)
