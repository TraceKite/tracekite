...
def computed_go_paths(content: str) -> int:
    ...
    """
    from tracekite.services.go_route_extractor import _scan, _imports
...
def framework_routes(file_info, content: str) -> tuple[list, int]:
    ...
    if path.endswith(".pb.gw.go"):
        ...
        from tracekite.services.grpc_gateway_routes import extract_gateway_routes
    ...
    if language in ("java", "kotlin", "scala"):
        from tracekite.services.jvm_route_extractor import extract_jvm_routes
    ...
    if language == "php" or path.endswith(".php"):
        from tracekite.services.php_route_extractor import extract_php_routes
        return extract_php_routes(path, content), 0
    if language in ("c#", "csharp") or path.endswith(".cs"):
        from tracekite.services.csharp_route_extractor import (
