# Evigraph Agent Plugin

This one plugin source supports Claude Code, Codex, and Kimi Code. It bundles
the Evigraph skill with an MCP server declaration; each client reads its own
manifest and ignores the others.

Every relationship returned by the MCP tools carries the evidence stored in
the graph. An empty or declined answer means Evigraph did not establish the
relationship.

## Prerequisite

Install the local Evigraph CLI before enabling the plugin:
...
The bundled MCP command is `evigraph mcp` with no explicit path. It scans the
...
```bash
claude --plugin-dir ./plugins/evigraph
```
...
claude plugin install evigraph@evigraph-labs
```
...
codex plugin add evigraph@evigraph-labs
```
...
```text
/plugins install ./plugins/evigraph
/reload
```
...
```bash
claude mcp add --scope user evigraph-estate -- evigraph mcp \
  /path/to/orders /path/to/billing /path/to/gateway

codex mcp add evigraph-estate -- evigraph mcp \
  /path/to/orders /path/to/billing /path/to/gateway
```
...
```text
What services does Evigraph know about?
...
[agent integration guide](https://github.com/evigraph-labs/evigraph/blob/main/docs/plugins-and-mcp-guide.md)
